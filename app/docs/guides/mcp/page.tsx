'use client';

import { Terminal, Cpu } from 'lucide-react';
import Link from 'next/link';

export default function MCPGuide() {
  return (
    <main className="min-h-screen bg-background text-foreground">
      <nav className="fixed top-0 w-full z-50 glass-morphic border-b border-border">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <Terminal className="w-6 h-6 text-accent-blue" />
            <span className="text-xl font-bold">stick.ai</span>
          </Link>
        </div>
      </nav>

      <div className="pt-24 pb-20">
        <div className="max-w-4xl mx-auto px-6">
          <Link href="/docs" className="text-sm text-accent-blue hover:underline mb-4 inline-block">
            ← Back to Docs
          </Link>
          <h1 className="text-5xl font-bold mb-4">
            <span className="gradient-text">MCP</span> Integration
          </h1>
          <p className="text-xl text-zinc-400 mb-12">
            Connect external tools via Model Context Protocol
          </p>

          <section className="glass-morphic rounded-lg p-6 border border-border mb-6">
            <h2 className="text-2xl font-bold mb-4 flex items-center gap-2">
              <Cpu className="w-6 h-6 text-accent-cyan" />
              What is MCP?
            </h2>
            <p className="text-zinc-400 mb-4">
              Model Context Protocol (MCP) allows AI agents to connect to external tools, databases, and services
              through a standardized interface.
            </p>
          </section>

          <section className="glass-morphic rounded-lg p-6 border border-border mb-6">
            <h2 className="text-2xl font-bold mb-4">Adding MCP Servers</h2>
            <p className="text-zinc-400 mb-4">Use interactive mode:</p>
            <pre className="bg-surface/50 border border-border rounded-md p-4 mb-4">
              <code className="text-accent-cyan">stick{'\n'}→ Add MCP Server</code>
            </pre>
            <p className="text-zinc-400 text-sm">Supports HTTP, WebSocket, and stdio protocols</p>
          </section>

          <section className="glass-morphic rounded-lg p-6 border border-border">
            <h2 className="text-2xl font-bold mb-4">Common MCP Servers</h2>
            <ul className="space-y-2 text-zinc-400">
              <li>• Filesystem operations</li>
              <li>• Database connections</li>
              <li>• API integrations</li>
              <li>• Custom tools</li>
            </ul>
          </section>
        </div>
      </div>
    </main>
  );
}
