...
async function configureTools(agentPath) {
    const configPath = agentPath
        ? path_1.default.join(agentPath, 'config', 'agent.json')
    ...
    console.log(chalk_1.default.gray(`Current tools: ${currentTools.join(', ') || 'none'}\n`));
    ...
    if (action === 'view') {
        ...
        AVAILABLE_TOOLS.forEach(tool => {
            const installed = currentTools.includes(tool.name) ? chalk_1.default.green('✓') : chalk_1.default.gray('○');
            console.log(`  ${installed} ${chalk_1.default.yellow(tool.name.padEnd(15))} ${chalk_1.default.gray(tool.category.padEnd(15))} ${tool.description}`);
        });
        console.log();
    ...
    if (action === 'remove') {
        ...
        if (tools.length > 0) {
            config.tools = currentTools.filter((t) => !tools.includes(t));
            await fs_extra_1.default.writeJson(configPath, config, { spaces: 2 });
            console.log(chalk_1.default.green(`\n✓ Removed ${tools.length} tool(s)\n`));
        }
    }
}
async function addMCP(agentPath) {
    const configPath = agentPath
        ? path_1.default.join(agentPath, 'config', 'agent.json')
        : path_1.default.join(process.cwd(), 'config', 'agent.json');
    if (!await fs_extra_1.default.pathExists(configPath)) {
    ...
    console.log(chalk_1.default.gray('Connect your agent to external tools and data sources via MCP\n'));
    ...
    console.log(chalk_1.default.gray('Your agent can now access tools from this MCP server\n'));
}
