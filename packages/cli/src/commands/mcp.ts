...
async function testMCPServer(serverName?: string) {
  ...
  const spinner = ora(`Testing ${serverName}...`).start();
  ...
  try {
    ...
    if (errorOutput.includes('running on stdio')) {
      ...
      console.log(chalk.gray('  Server output:', errorOutput.split('\n')[0]));
    } else if (output || errorOutput) {
      spinner.succeed('Server test completed');
      console.log(chalk.cyan('\nServer output:'));
      if (errorOutput) console.log(chalk.gray(errorOutput));
      if (output) console.log(chalk.white(output));
    } else {
    ...
    console.log(chalk.cyan('\nNext steps:'));
    console.log(chalk.white(`  1. Add to agent config:`));
